                              ...
                              uint8_t *final_key)
{
    ...
    if (psa_ret != PSA_SUCCESS) {
        fprintf(stderr, "\tt = KDF(s, 0x00) failed, psa_status_t=%ld\n", psa_ret);
        goto exit;
    }
    ...
    if (psa_ret != PSA_SUCCESS) {
        fprintf(stderr, "\tu = KDF(s, 0x01) failed, psa_status_t=%ld\n", psa_ret);
        goto exit;
    }
    ...
    if (psa_ret != PSA_SUCCESS) {
        fprintf(stderr, "\tv = KDF(0, PIN||A) failed, psa_status_t=%ld\n", psa_ret);
        goto exit;
    }
    ...
    for (int i = 0; i < nvm.i; i++) {
        ...
        if (psa_ret != PSA_SUCCESS) {
            fprintf(stderr, "\tk_i = KDF(w_i, PIN||A) failed, psa_status_t=%ld\n", psa_ret);
            goto exit;
        }
    ...
    if (psa_ret != PSA_SUCCESS) {
        fprintf(stderr, "\tFail during last computation of final_key, psa_status_t=%ld\n", psa_ret);
        goto exit;
    }
                                ...
                                const uint8_t *add, const uint8_t add_size, uint8_t *final_key)
{
    ...
    psa_ret = hmac_sha256(w_i, sizeof(w_i), kdf_input_buff, PIN_size + add_size_checked, k_i);
    if (psa_ret != PSA_SUCCESS) {
        fprintf(stderr, "\tk'_i = KDF(w', PIN'||A) failed, psa_status_t=%ld\n", psa_ret);
        goto exit;
    }
    ...
    if (psa_ret != PSA_SUCCESS) {
        fprintf(stderr, "\tt = KDF(s_, \"0x00\") failed, psa_status_t=%ld\n", psa_ret);
        goto exit;
    }
...
/**
 * @brief Initialize the RNG and PSA Crypto subsystem, configure the device, and open a Secure
 * Session. On success all three output parameters are fully initialized and linked. On failure
 * the caller does not need to perform any cleanup.
 *
 * @param handle      Zeroed lt_handle_t allocated by the caller.
 * @param device      Zeroed lt_dev_stm32u5xx_t allocated by the caller.
 * @param crypto_ctx  Uninitialized lt_ctx_mbedtls_v4_t allocated by the caller.
 * @return 0 on success, -1 on failure.
 */
static int lt_start_session(lt_handle_t *handle, lt_dev_stm32u5xx_t *device,
                             lt_ctx_mbedtls_v4_t *crypto_ctx)
{
    ...
    if (HAL_RNG_GetState(&hrng) != HAL_RNG_STATE_READY) {
        fprintf(stderr, "RNG peripheral is not ready! Make sure to initialize it before using any Libtropic function, ret=%d\n", (int)HAL_RNG_GetState(&hrng));
        return -1;
    }
    ...
    ret = lt_verify_chip_and_start_secure_session(handle, DEFAULT_SH0_PRIV, DEFAULT_SH0_PUB,
...
{
    if (lt_start_session(&lt_handle, &device, &crypto_ctx) != 0) {
        return -1;
    }
    ...
    ret = new_PIN_setup(&lt_handle, master_secret, pin, sizeof(pin), NULL,
                        0, final_key_initialized);
    if (ret != LT_OK) {
...
{
    return PIN_entry_check(&lt_handle, pin, pin_size, NULL, 0, (uint8_t[TR01_MAC_AND_DESTROY_DATA_SIZE]){0});
}